...
void selectPopConfig(const std::string& network, bool enable_finalization)
{
    ...
    if (network == CBaseChainParams::MAIN) {
        ...
        auto altparam = std::make_shared<VeriBlock::AltChainParamsBTCSQ>(Params().GenesisBlock(), enable_finalization);
        ...
        auto altparam = std::make_shared<VeriBlock::AltChainParamsBTCSQRegTest>(Params().GenesisBlock(), enable_finalization);
        popconfig.alt = altparam;
